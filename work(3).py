...
def viterbi( filename ):
    ...
    for i in range(0,n,2):
        # each element is the probability and the state path
        Pi = {state: [0.0, []] for state in state_dict}
        if i==0:
            # only one observation for the first
            observation = sequence[i]
            for state in state_dict:
                # its probability is Pai(state) * P(observation | state)
                Pi[state][0] = state_p[state] * state_observation_p[(state,observation)]
                # its path is [state]
                Pi[state][1] = [state]
        else:
            # one action and one observation
            action, observation = sequence[i-1], sequence[i]
            # for each state, get the max probability
            for state in state_dict:
                max_p, max_path = 0.0, []
                # go through all the states for the last step, get the max one
                for last_state in state_dict:
                    # last_state -> action -> state (observation)
                    p, path = P[-1][last_state][0], [ s for s in P[-1][last_state][1] ]
                    # P(state | (last_state,action)) * P(observation | state)
    ...
    # go through all the states as the last location, get the max probability
